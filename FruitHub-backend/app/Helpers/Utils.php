	...
	static public function createDataTableResult(Request $request, $model, $wheres, $columnsToFilter, $columnsRelationshipToFilter = null, $sortField = '', $sortOrder = 'asc', $with = null, $join = null) {
		...
		foreach ($result as &$r) {
			if (!empty($r->created_at)) {
				$r->created_at_format = Carbon::parse($r->created_at)->format('d/m/Y H:i:s');
			}

			if (!empty($r->updated_at)) {
				$r->updated_at_format = Carbon::parse($r->updated_at)->format('d/m/Y H:i:s');
			}

			if (!empty($r->met_data_disponivel)) {
				$r->met_data_disponivel = Carbon::parse($r->met_data_disponivel)->format('d/m/Y H:i:s');
			}

			if (!empty($r->not_data_disponivel)) {
				$r->not_data_disponivel = Carbon::parse($r->not_data_disponivel)->format('d/m/Y H:i:s');
			}

			if (!empty($r->pes_data_inicial)) {
				$r->pes_data_inicial = Carbon::parse($r->pes_data_inicial)->format('d/m/Y H:i:s');
			}

			if (!empty($r->pes_data_final)) {
				$r->pes_data_final = Carbon::parse($r->pes_data_final)->format('d/m/Y H:i:s');
			}
		}
